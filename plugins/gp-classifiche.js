...
classifica.forEach(([jid,d],i)=>{testo+=`
...
await conn.sendMessage(m.chat,{text:testo,mentions:menzioni,footer:isAll?'Classifica storica del gruppo':'Classifica giornaliera del gruppo',buttons,headerType:1},{quoted:m})
}
...
let chat=global.db.data.chats[m.chat],oggi=dataKeyRoma()
ensureChat(chat,oggi)
if(!chat.classificaGiornaliera||chat.classificaGiornaliera.ultimoReset!==oggi)chat.classificaGiornaliera={totali:0,utenti:{},ultimoReset:oggi}
let g=chat.classificaGiornaliera,t=chat.classificaTotale
g.totali++;g.utenti[m.sender]??={conteggio:0};g.utenti[m.sender].conteggio++
t.totali++;t.utenti[m.sender]??={conteggio:0};t.utenti[m.sender].conteggio++
}
